...
for i, line in enumerate(lines):
    if line.startswith("Monkey"):
        ...
        monkey = Monkey(
            ...
            operation = lambda x, operator=operator, y=y: operator(
                x, x if y == "old" else int(y)
            ),
            test = lambda x, modulo=modulo, test_true=test_true, test_false=test_false: test_true 
            if x % modulo == 0 
            else test_false,
        )
        monkeys.append(monkey)
        common_multiples.append(modulo)
...
def part_2():
    common_multiple = reduce((lambda x, y: x * y), common_multiples)

    for i in range(0, 10000):
        for monkey in monkeys:
            for item in monkey.items:
                item_after_inspection = monkey.operation(item) % common_multiple
                next_monkey = monkey.test(item_after_inspection)
                monkeys[next_monkey].items.append(item_after_inspection)
                monkey.inspect_count += 1
            monkey.items = []
